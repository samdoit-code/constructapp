...
- **`readSheet_` must never write.** It used to stamp missing `id`s as a side effect of reading, which is why `getAll` had to hold the write lock. That is now a separate, explicit `backfillMissingIds_(key)` pass that touches only column A, runs once per sheet inside a short-lived lock at the top of `getAll`, and leaves the actual reads lock-free and side-effect-free. **Reusable pattern:** a read path that mutates is a read path that has to be serialised — keep repair/migration work in its own named, explicitly-invoked pass instead of hiding it inside the reader.
