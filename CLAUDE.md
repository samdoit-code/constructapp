...
- **Service worker (`sw.js`):** deliberately network-first, not cache-first — it only serves its cached copy of `index.html`/assets when a fetch genuinely fails (true offline), never to mask a slow-but-successful network response. It explicitly never intercepts `version.json` (must always hit the network, see `checkForUpdate()`) or any cross-origin request (the Apps Script backend). This exists purely so a fully offline launch can still boot the app shell and hand off to the existing `localStorage` data cache — it must never become the reason an online user sees stale content after a real deploy. If this is ever made more aggressive (e.g. cache-first), re-verify `forceAppReload()`'s cache-busting still works against it.
